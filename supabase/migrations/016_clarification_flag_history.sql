-- Clarification flags become status-based history instead of being deleted
-- on dismiss/resolve, so an Approver can see that something was flagged even
-- after Sales dismissed it or resolved it by editing the section — per
-- feedback, that's still important information for the Approver to have,
-- not just whatever is still open at review time.
--
-- dismiss_clarification_flag now marks the matching flag "dismissed"
-- instead of removing it from the array. create_proposal_version now
-- requires every flag to carry a status, and derives next_status from
-- whether any flag is still "open" (not from the array's length, which no
-- longer shrinks to zero just because everything on it was resolved).
...
begin
  ...
  select coalesce(jsonb_agg(
    case when elem->>'id' = p_flag_id and elem->>'status' = 'open'
         then elem || jsonb_build_object('status', 'dismissed')
         else elem
    end
  ), '[]'::jsonb)
  into v_next_flags
  ...
  select exists (
    select 1 from jsonb_array_elements(v_next_flags) e where e->>'status' = 'open'
  ) into v_has_open;

  if not v_has_open and v_proposal.status = 'needs_clarification' then
...
-- ---------------------------------------------------------------------------
-- create_proposal_version — flags now require a status; next_status derives
-- from "any flag still open", not array length.
-- ---------------------------------------------------------------------------
...
begin
  ...
  for v_flag in select jsonb_array_elements(v_flags) loop
    ...
    if v_flag->>'status' not in ('open', 'dismissed', 'resolved') then
      raise exception 'VALIDATION_ERROR: invalid clarification flag status %', v_flag->>'status';
    end if;
  end loop;

  -- Mirrors lib/domain/state-machine.ts computeEditableStatus (fed only
  -- "open" flags — see lib/domain/clarification.ts openClarificationFlags):
  -- a version moves to needs_clarification exactly when a flag is still
  -- open, regardless of how many dismissed/resolved ones are also on it.
  select exists (
    select 1 from jsonb_array_elements(v_flags) e where e->>'status' = 'open'
  ) into v_has_open;
  v_derived_next_status := case when v_has_open then 'needs_clarification' else 'draft' end;
